...
public abstract class Animation implements Cloneable {
    ...
    public Animation(Context context, AttributeSet attrs) {
        TypedArray a = context.obtainStyledAttributes(attrs, com.glview.R.styleable.Animation);

        setDuration((long) a.getInt(com.glview.R.styleable.Animation_duration, 0));
        setStartOffset((long) a.getInt(com.glview.R.styleable.Animation_startOffset, 0));
        
        setFillEnabled(a.getBoolean(com.glview.R.styleable.Animation_fillEnabled, mFillEnabled));
        setFillBefore(a.getBoolean(com.glview.R.styleable.Animation_fillBefore, mFillBefore));
        setFillAfter(a.getBoolean(com.glview.R.styleable.Animation_fillAfter, mFillAfter));

        setRepeatCount(a.getInt(com.glview.R.styleable.Animation_repeatCount, mRepeatCount));
        setRepeatMode(a.getInt(com.glview.R.styleable.Animation_repeatMode, RESTART));

        setZAdjustment(a.getInt(com.glview.R.styleable.Animation_zAdjustment, ZORDER_NORMAL));
        
        setBackgroundColor(a.getInt(com.glview.R.styleable.Animation_gl_Background, 0));

        setDetachWallpaper(a.getBoolean(com.glview.R.styleable.Animation_detachWallpaper, false));

        final int resID = a.getResourceId(com.glview.R.styleable.Animation_interpolator, 0);
    ...
    public void reverse(){
		...
		if (timeLeft <= 0) mStartTime = -1;
		else mStartTime = currentTime - timeLeft;
    }
...
//    	setInterpolator(yunos.tv.view.animation.AnimationUtils.loadInterpolator(context, resID));
    }
    ...
    public void setStartTime(long startTimeMillis) {
        ...
        if(startTimeMillis == START_ON_FIRST_FRAME){
        	mRerverse = false;
        }
    }
     ...
     */
    public boolean getTransformation(long currentTime, GLCanvas canvas) {
        if (mStartTime == -1) {
    ...
    public boolean getTransformation(long currentTime,
            float scale, GLCanvas canvas) {
        mScaleFactor = scale;
        return getTransformation(currentTime, canvas);
    }
     ...
     *        transforms.
     */
    protected void applyTransformation(float interpolatedTime, GLCanvas canvas) {
    }
