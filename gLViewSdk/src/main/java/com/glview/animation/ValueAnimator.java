...
import com.glview.view.animation.AccelerateDecelerateInterpolator;
...
public class ValueAnimator extends Animator {
    ...
    protected static WeakHashMap<Looper, AnimationHandler> sAnimationHandler = new WeakHashMap<Looper, AnimationHandler>();
...
//                mChoreographer.postCallback(Choreographer.CALLBACK_ANIMATION, this, null);
    			...
    			if (!mHandler.post(this)) {
    				clearAllAnimations();
        			Log.w("AnimationHandler", "Handler Thread is not started or has been quit!");
    			}
            }
        }
    }
    ...
    public static int getCurrentAnimationsCount() {
        AnimationHandler handler = getOrCreateAnimationHandler();//sAnimationHandler.get();
    ...
    protected static AnimationHandler getOrCreateAnimationHandler() {
        ...
        return handler;*/
    	Looper token = getAnimationLooper();
    	AnimationHandler handler = sAnimationHandler.get(token);
    	if (handler == null) {
    		handler = new AnimationHandler(token);
    		sAnimationHandler.put(token, handler);
    	}
    	return handler;
    }
    ...
    /**
     * TODO 
     * If we are now in RenderThread, run the animation in RenderThread, otherwise run 
     * the animation in GLThread.
     * @return
     * @author lijing.lj
     */
    private final static Looper getAnimationLooper() {
    	Looper looper = Looper.myLooper();
    	if (looper != null && looper.getThread().isAlive() && "com.glview.hwui.RenderThread".equals(looper.getThread().getClass().getName())) {
    		return looper;
    	}
