...
import com.glview.libgdx.graphics.utils.reflect.ArrayReflection;
...
public class Array<T> implements Iterable<T> {
	...
	public T selectRanked (Comparator<T> comparator, int kthLowest) {
		if (kthLowest < 1) {
			throw new GLViewRuntimeException("nth_lowest must be greater than 0, 1 = first, 2 = second...");
		}
	...
	static public class ArrayIterator<T> implements Iterator<T>, Iterable<T> {
		...
		public T next () {
			...
			if (!valid) throw new GLViewRuntimeException("#iterator() cannot be used nested.");
			return array.items[index++];
		}

		public void remove () {
			if (!allowRemove) throw new GLViewRuntimeException("Remove not allowed.");
			index--;
