...
import com.glview.libgdx.graphics.math.Matrix3;
 ...
 * 
 * <p>
 * After construction a ShaderProgram can be used to draw {@link Mesh_GL10}. To make the GPU use a specific ShaderProgram the programs
...
// }
	...
	private int linkProgram () {
		...
		if (linked == 0) {
// App.getGL20().glGetProgramiv(program, GL20.GL_INFO_LOG_LENGTH, intbuf);
// int infoLogLength = intbuf.get(0);
// if (infoLogLength > 1) {
			log = App.getGL20().glGetProgramInfoLog(program);
// }
			return -1;
		}
...
// }
	...
	public int fetchAttributeLocation(HandleInfo handle){
		if(!handle.checkLocationValid()){
			checkManaged();
			int location = fetchAttributeLocation (handle.name);
	...
	private int fetchUniformLocation (HandleInfo handle) {
		if(!handle.checkLocationValid()){
			checkManaged();
			int location = fetchUniformLocation (handle.name);
			handle.location = location;
		}		
		return handle.location;
	}
	...
	public void setUniformi (HandleInfo handle, int value) {
		checkManaged();
		int location = fetchUniformLocation(handle);
	...
	public void setUniformi (HandleInfo handle, int value1, int value2) {
		checkManaged();
		int location = fetchUniformLocation(handle);
	...
	public void setUniformi (HandleInfo handle, int value1, int value2, int value3) {
		checkManaged();
		int location = fetchUniformLocation(handle);
	...
	public void setUniformi (HandleInfo handle, int value1, int value2, int value3, int value4) {
		checkManaged();
		int location = fetchUniformLocation(handle);
	...
	public void setUniformf (HandleInfo handle, float value) {
		checkManaged();
		int location = fetchUniformLocation(handle);
	...
	public void setUniformf (HandleInfo handle, float value1, float value2) {
		checkManaged();
		int location = fetchUniformLocation(handle);
		setUniformf (location, value1, value2);
	}
	...
	public void setUniformf (HandleInfo handle, float value1, float value2, float value3) {
		checkManaged();
		int location = fetchUniformLocation(handle);
		setUniformf (location, value1, value2, value3);
	}
	...
	public void setUniformf (HandleInfo handle, float value1, float value2, float value3, float value4) {
		checkManaged();
		int location = fetchUniformLocation(handle);
		setUniformf (location, value1, value2, value3, value4);
	}
	...
	public void setUniform1fv (HandleInfo handle, float[] values, int offset, int length) {
		checkManaged();
		int location = fetchUniformLocation(handle);
	...
	public void setUniform2fv (HandleInfo handle, float[] values, int offset, int length) {
		checkManaged();
		int location = fetchUniformLocation(handle);
	...
	public void setUniform3fv (HandleInfo handle, float[] values, int offset, int length) {
		checkManaged();
		int location = fetchUniformLocation(handle);
	...
	public void setUniform4fv (HandleInfo handle, float[] values, int offset, int length) {
		checkManaged();
		int location = fetchUniformLocation(handle);
	...
	public void setUniformMatrix (HandleInfo handle, Matrix4 matrix) {
		checkManaged();
		int location = fetchUniformLocation(handle);
		setUniformMatrix(location, matrix, false);
	}
	...
	public void setUniformMatrix (HandleInfo handle, Matrix4 matrix, boolean transpose) {
		checkManaged();
		int location = fetchUniformLocation(handle);
	...
	public void setUniformMatrix (HandleInfo handle, Matrix3 matrix) {
		setUniformMatrix(handle, matrix, false);
	}
	...
	public void setUniformMatrix (HandleInfo handle, Matrix3 matrix, boolean transpose) {
		checkManaged();
		int location = fetchUniformLocation(handle);
		setUniformMatrix(location, matrix, transpose);
	}
	...
	public void setUniformMatrix4fv (HandleInfo handle, float[] values, int offset, int length) {
		checkManaged();
		int location = fetchUniformLocation(handle);
		
		setUniformMatrix4fv (location, values, offset, length);
	}
	...
	public void setUniformf (HandleInfo handle, Vector2 values) {
		setUniformf(handle, values.x, values.y);
	}
	...
	public void setUniformf (HandleInfo handle, Vector3 values) {
		setUniformf(handle, values.x, values.y, values.z);
	}
	...
	public void setUniformf (HandleInfo handle, Color values) {
		setUniformf(handle, values.r, values.g, values.b, values.a);
	}
	...
	public void setVertexAttribute (HandleInfo handle, int size, int type, boolean normalize, int stride, Buffer buffer) {
		checkManaged();
		int location = fetchAttributeLocation(handle);
	...
	public void setVertexAttribute (HandleInfo handle, int size, int type, boolean normalize, int stride, int offset) {
		checkManaged();
		int location = fetchAttributeLocation(handle);
		
		setVertexAttribute(location, size, type, normalize, stride, offset);
	}
	...
	public void disableVertexAttribute (HandleInfo handle) {
		checkManaged();
	...
	public void enableVertexAttribute (HandleInfo handle) {
		checkManaged();
	...
	static final int[] lengthArray = new int[1];
	static final int[] sizeArray = new int[1];
	static final int[] typeArray = new int[1];
	static final byte[] nameArray = new byte[500];

	private void fetchUniforms () {
		params.clear();
		App.getGL20().glGetProgramiv(program, GL20.GL_ACTIVE_UNIFORMS, params);
		int numUniforms = params.get(0);
		
		uniformNames = new String[numUniforms];
		
		for (int i = 0; i < numUniforms; i++) {
			String name = getActiveUniform(i);
			int location = App.getGL20().glGetUniformLocation(program, name);
			uniforms.put(name, location);
	...
	private String getActiveUniform(int index) {
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
			params.clear();
			params.put(0, 1);
			type.clear();
			String name = App.getGL20().glGetActiveUniform(program, index, params, type);
			return name;
		} else {
			params.clear();
			type.clear();
			App.getGL20().glGetActiveUniform(program, index, 500, lengthArray, 0, sizeArray, 0, typeArray, 0, nameArray, 0);
			String name = new String(nameArray, 0, lengthArray[0]);
			type.put(0, typeArray[0]);
			params.put(0, sizeArray[0]);
			return name;
		}
	}
	...
	private void fetchAttributes () {
		params.clear();
		App.getGL20().glGetProgramiv(program, GL20.GL_ACTIVE_ATTRIBUTES, params);
		int numAttributes = params.get(0);

		attributeNames = new String[numAttributes];

		for (int i = 0; i < numAttributes; i++) {
			String name = getActiveAttrib(i);
			int location = App.getGL20().glGetAttribLocation(program, name);
			attributes.put(name, location);
	...
	private String getActiveAttrib(int index) {
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
			params.clear();
			params.put(0, 1);
			type.clear();
			String name = App.getGL20().glGetActiveAttrib(program, index, params, type);
			return name;
		} else {
			params.clear();
			type.clear();
			App.getGL20().glGetActiveAttrib(program, index, 200, lengthArray, 0, sizeArray, 0, typeArray, 0, nameArray, 0);
			String name = new String(nameArray, 0, lengthArray[0]);
			type.put(0, typeArray[0]);
			params.put(0, sizeArray[0]);
			return name;
	...
	static public class HandleInfo{
		public String name;
		public static final int INVALID = -1;
		public int location = INVALID;
		public boolean checkLocationValid(){
			return location == INVALID ? false : true;
		}
		
		public HandleInfo(String name){
			this.name = name;
		}
