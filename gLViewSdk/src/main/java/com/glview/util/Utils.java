...
public class Utils {
    ...
    public static void assertTrue(boolean cond) {
    	//TODO:为了保证桌面不crash，暂时先不抛出异常，看界面是否有什么异常
        if (!cond) {
//            throw new AssertionError();
        	Log.e("GLRootView", "---------asserTrue", new Exception());
        }
    }
    ...
    public static void initPath(Context context){
        ...
        for (StorageVolume sv : storageVolumes) {
            if(num > 2) { // first two volumes.
                ...
                OTHERS_SDCARD_PATH = sv.getPath();
            } else {
                if (sv.isRemovable()) {
                	EXTERNAL_SDCARD_PATH = sv.getPath();
                } else {
                    INTERNAL_SDCARD_PATH = sv.getPath();
                }
            }
            num++;
        }
    }
    private static StorageVolume[] getVolumeList(){
        if(mStorageManager == null) return null;
        StorageVolume[] storageVolumes = null;
        try{
            storageVolumes = (StorageVolume[]) mStorageManager.getClass().getMethod("getVolumeList").invoke(mStorageManager);
    ...
    public static File getExternalStorage2Directory(){
    	return getDirectory("SECONDARY_STORAGE", "/storage/sdcard1");
    }
    ...
    public static boolean isExternalSDExist(){
    	File sdFile = new File(EXTERNAL_SDCARD_PATH);
    	if(sdFile.exists() && sdFile.getTotalSpace() > 0)
    		return true;
    	return false;
    }
