...
public class StaticLayout extends Layout {
    ...
    /* package */ void generate(CharSequence source, int bufStart, int bufEnd,
        ...
        GLPaint.FontMetricsInt fm = mFontMetricsInt;
...
//            breakOpp = nLineBreakOpportunities(localeLanguageTag, chs, paraEnd - paraStart, breakOpp);
            ...
            for (int i = 0; i < paraEnd - paraStart; i ++) {
            	char c = chs[i];
        		if (TextUtils.isWordBreak(c)) {
        			vec.add(i);
        		}
            }
            vec.add(-1);
            breakOpp = new int[vec.size()];
            for (int i = 0; i < vec.size(); i ++) {
            	breakOpp[i] = vec.get(i);
            }
    ...
    private int out(CharSequence text, int start, int end,
                      ...
                      GLPaint paint, boolean moreChars) {
        int j = mLineCount;
        int off = j * mColumns;
...
//        }
    ...
    private void calculateEllipsis(int lineStart, int lineEnd,
                                   ...
                                   int line, float textWidth, GLPaint paint,
...
//    private static native int[] nLineBreakOpportunities(String locale, char[] text, int length, int[] recycle);
    ...
    private GLPaint.FontMetricsInt mFontMetricsInt = new GLPaint.FontMetricsInt();
}
