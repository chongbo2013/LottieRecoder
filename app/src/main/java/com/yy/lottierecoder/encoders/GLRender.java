...
public class GLRender implements OnTextureAcceptableListener {
    ...
    protected void initShaderHandles() {
        mPositionHandle = GLES30.glGetAttribLocation(mProgramHandle, ATTRIBUTE_POSITION);
        mTextureCoordHandle = GLES30.glGetAttribLocation(mProgramHandle, ATTRIBUTE_TEXTURE_COORD);

        mTextureHandle = GLES30.glGetUniformLocation(mProgramHandle, UNIFORM_TEXTURE_0);
    }
    ...
    protected void bindShaderVertices() {
        ...
        GLES30.glVertexAttribPointer(mTextureCoordHandle, 2, GLES30.GL_FLOAT, false,
    ...
    protected void bindShaderTextures() {
        ...
        GLES30.glBindTexture(GLES30.GL_TEXTURE_2D, mTextureIn);
    ...
    protected void initGLContext() {
        ...
        if (!TextUtils.isEmpty(vertexShader) && !TextUtils.isEmpty(fragmentShader)) {
            // 初始化顶点着色器
            mVertexShaderHandle = ShaderHelper.compileShader(vertexShader, GLES30.GL_VERTEX_SHADER);
            // 初始化片元着色器
            mFragmentShaderHandle = ShaderHelper.compileShader(fragmentShader, GLES30.GL_FRAGMENT_SHADER);
